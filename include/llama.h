...
extern "C" {
    ...
    LLAMA_API bool                   llama_get_kv_cells(struct llama_context * ctx, uint32_t * size_out,
                                                        struct llama_kv_cell ** cells_out);

    // Get number of elements in the seq_id set for a specific cell
    LLAMA_API size_t llama_get_kv_cell_seq_id_count(
        const struct llama_kv_cell * cell);
