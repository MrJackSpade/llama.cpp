...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(common_arg({ "--no-display-prompt" },
                       string_format("don't print prompt at generation (default: %s)",
                                     !params.display_prompt ? "true" : "false"),
                       [](common_params & params) { params.display_prompt = false; })
    ...
    add_opt(common_arg({ "-lcs", "--lookup-cache-static" }, "FNAME",
        "path to static lookup cache to use for lookup decoding (not updated by generation)",
                       [](common_params & params, const std::string & value) { params.lookup_cache_static = value; })
                .set_examples({ LLAMA_EXAMPLE_LOOKUP }));
    add_opt(common_arg({ "-lcd", "--lookup-cache-dynamic" }, "FNAME",
        "path to dynamic lookup cache to use for lookup decoding (updated by generation)",
                       [](common_params & params, const std::string & value) { params.lookup_cache_dynamic = value; })
                .set_examples({ LLAMA_EXAMPLE_LOOKUP }));
    add_opt(common_arg({ "-c", "--ctx-size" }, "N",
        string_format("size of the prompt context (default: %d, 0 = loaded from model)", params.n_ctx),
                       [](common_params & params, int value) { params.n_ctx = value; })
    ...
    add_opt(common_arg({ "--cache-ram", "-cram" }, "N",
        string_format("set the maximum cache size in MiB (default: %d, -1 - no limit, 0 - disable)\n"
                ...
                .set_examples({ LLAMA_EXAMPLE_SERVER }));
    add_opt(common_arg({ "--kv-unified", "-kvu" },
        string_format("use single unified KV buffer for the KV cache of all sequences (default: %s)\n"
                                     "[(more info)](https://github.com/ggml-org/llama.cpp/pull/14363)",
                                     params.kv_unified ? "true" : "false"),
                       [](common_params & params) { params.kv_unified = true; })
                .set_env("LLAMA_ARG_KV_UNIFIED"));
    add_opt(
        common_arg({ "--no-context-shift" },
                   string_format("disables context shift on infinite text generation (default: %s)",
                                 params.ctx_shift ? "disabled" : "enabled"),
                   [](common_params & params) { params.ctx_shift = false; })
            .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_IMATRIX, LLAMA_EXAMPLE_PERPLEXITY })
            .set_env("LLAMA_ARG_NO_CONTEXT_SHIFT"));
    add_opt(
        common_arg({ "--context-shift" },
                   string_format("enables context shift on infinite text generation (default: %s)",
                                 params.ctx_shift ? "enabled" : "disabled"),
                   [](common_params & params) { params.ctx_shift = true; })
            .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_IMATRIX, LLAMA_EXAMPLE_PERPLEXITY })
    ...
    add_opt(common_arg({ "--no-perf" },
                       string_format("disable internal libllama performance timings (default: %s)",
    ...
    add_opt(common_arg({ "-sysf", "--system-prompt-file" }, "FNAME",
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (!params.system_prompt.empty() && params.system_prompt.back() == '\n') {
                ...
                .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_DIFFUSION }));
    add_opt(common_arg({ "--in-file" }, "FNAME", "an input file (repeat to specify multiple files)",
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "-bf", "--binary-file" }, "FNAME", "binary file containing the prompt (default: none)",
        [](common_params & params, const std::string & value) {
            ...
            fprintf(stderr, "Read %zu bytes from binary file %s\n", params.prompt.size(), value.c_str());
                ...
                .set_excludes({ LLAMA_EXAMPLE_SERVER }));
    add_opt(common_arg({ "-e", "--escape" },
                       string_format("process escapes sequences (\\n, \\r, \\t, \\', \\\", \\\\) (default: %s)",
                                     params.escape ? "true" : "false"),
                       [](common_params & params) { params.escape = true; }));
    add_opt(common_arg({ "--no-escape" }, "do not process escape sequences",
                       [](common_params & params) { params.escape = false; }));
    ...
    add_opt(common_arg({ "-sp", "--special" },
        string_format("special tokens output enabled (default: %s)", params.special ? "true" : "false"),
                ...
                .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_SERVER }));
    add_opt(common_arg({ "-cnv", "--conversation" },
        "run in conversation mode:\n"
        "- does not print special tokens and suffix/prefix\n"
        "- interactive mode is also enabled\n"
        "(default: auto enabled if chat template is available)",
                       [](common_params & params) { params.conversation_mode = COMMON_CONVERSATION_MODE_ENABLED; })
                .set_examples({ LLAMA_EXAMPLE_MAIN }));
    add_opt(common_arg({ "-no-cnv", "--no-conversation" }, "force disable conversation mode (default: false)",
                       [](common_params & params) { params.conversation_mode = COMMON_CONVERSATION_MODE_DISABLED; })
                .set_examples({ LLAMA_EXAMPLE_MAIN }));
    ...
    add_opt(common_arg({ "--in-suffix" }, "STRING", "string to suffix after user inputs with (default: empty)",
        [](common_params & params, const std::string & value) {
            ...
            params.enable_chat_template = false;
                ...
                .set_examples({ LLAMA_EXAMPLE_MAIN }));
    add_opt(common_arg({ "--no-warmup" }, "skip warming up the model with an empty run",
                       [](common_params & params) { params.warmup = false; })
                .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_MTMD, LLAMA_EXAMPLE_EMBEDDING,
                                LLAMA_EXAMPLE_RETRIEVAL, LLAMA_EXAMPLE_PERPLEXITY }));
    ...
    add_opt(
        ...
        string_format("RNG seed (default: %d, use random seed for %d)", params.sampling.seed, LLAMA_DEFAULT_SEED),
            ...
            .set_sparam());
    add_opt(common_arg({ "--sampling-seq", "--sampler-seq" }, "SEQUENCE",
    ...
    add_opt(common_arg({ "--ignore-eos" },
        "ignore end of stream token and continue generating (implies --logit-bias EOS-inf)",
                ...
                .set_sparam());
    add_opt(common_arg({ "--temp" }, "N", string_format("temperature (default: %.1f)", (double) params.sampling.temp),
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "--xtc-probability" }, "N",
                       string_format("xtc probability (default: %.1f, 0.0 = disabled)",
    ...
    add_opt(
        common_arg({ "--typical" }, "N",
                   string_format("locally typical sampling, parameter p (default: %.1f, 1.0 = disabled)",
    ...
    add_opt(common_arg({ "--repeat-penalty" }, "N",
                       string_format("penalize repeat sequence of tokens (default: %.1f, 1.0 = disabled)",
    ...
    add_opt(common_arg({ "--presence-penalty" }, "N",
                       string_format("repeat alpha presence penalty (default: %.1f, 0.0 = disabled)",
    ...
    add_opt(common_arg({ "--frequency-penalty" }, "N",
                       string_format("repeat alpha frequency penalty (default: %.1f, 0.0 = disabled)",
    ...
    add_opt(common_arg({ "--dry-multiplier" }, "N",
                       string_format("set DRY sampling multiplier (default: %.1f, 0.0 = disabled)",
    ...
    add_opt(common_arg({ "--dynatemp-range" }, "N",
                       string_format("dynamic temperature range (default: %.1f, 0.0 = disabled)",
    ...
    add_opt(common_arg({ "--dynatemp-exp" }, "N",
                       string_format("dynamic temperature exponent (default: %.1f)",
    ...
    add_opt(common_arg({ "--mirostat-lr" }, "N",
                       string_format("Mirostat learning rate, parameter eta (default: %.1f)",
    ...
    add_opt(common_arg({ "--mirostat-ent" }, "N",
                       string_format("Mirostat target entropy, parameter tau (default: %.1f)",
        ...
        [](common_params & params, const std::string & value) {
            params.sampling.mirostat_tau = std::stof(value);
                ...
                .set_sparam());
    add_opt(
        common_arg({ "--power-law-max-target" }, "N",
                   string_format("Power law max target (default: %.1f)", (double) params.sampling.power_law_max_target),
                   [](common_params & params, const std::string & value) {
                       params.sampling.power_law_max_target = std::stof(value);
                   })
            .set_sparam());
    add_opt(
        common_arg({ "--power-law-min-target" }, "N",
                   string_format("Power law min target (default: %.1f)", (double) params.sampling.power_law_min_target),
                   [](common_params & params, const std::string & value) {
                       params.sampling.power_law_min_target = std::stof(value);
                   })
            .set_sparam());
    add_opt(common_arg({ "--power-law-target" }, "N",
                       string_format("Power law target (default: %.1f)", (double) params.sampling.power_law_target),
                       [](common_params & params, const std::string & value) {
                           params.sampling.power_law_target = std::stof(value);
                       })
                .set_sparam());
    add_opt(common_arg({ "--power-law-queue-size" }, "N",
                       string_format("Power law queue size (default: %d)", params.sampling.power_law_queue_size),
                       [](common_params & params, int value) { params.sampling.power_law_queue_size = value; })
                .set_sparam());
    add_opt(common_arg({ "--power-law-dist-width" }, "N",
                       string_format("Power law distribution width (default: %.1f)",
                                     (double) params.sampling.power_law_distribution_width),
                       [](common_params & params, const std::string & value) {
                           params.sampling.power_law_distribution_width = std::stof(value);
                       })
                .set_sparam());
    add_opt(common_arg({ "--power-law-peak-logit" }, "N",
                       string_format("Power law peak logit value (default: %.1f)",
                                     (double) params.sampling.power_law_peak_logit_value),
                       [](common_params & params, const std::string & value) {
                           params.sampling.power_law_peak_logit_value = std::stof(value);
                       })
                .set_sparam());
    add_opt(common_arg({ "--power-law-tail-heaviness" }, "N",
                       string_format("Power law tail heaviness (default: %.1f)",
                                     (double) params.sampling.power_law_tail_heaviness),
                       [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "--pooling" }, "{none,mean,cls,last,rank}",
        ...
        [](common_params & params, const std::string & value) {
                ...
                .set_examples({ LLAMA_EXAMPLE_EMBEDDING, LLAMA_EXAMPLE_RETRIEVAL, LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(
        common_arg({ "--yarn-ext-factor" }, "N",
                   string_format("YaRN: extrapolation mix factor (default: %.1f, 0.0 = full interpolation)",
                                 (double) params.yarn_ext_factor),
                   [](common_params & params, const std::string & value) { params.yarn_ext_factor = std::stof(value); })
            .set_env("LLAMA_ARG_YARN_EXT_FACTOR"));
    add_opt(common_arg(
        {"--yarn-attn-factor"}, "N",
                string_format("YaRN: scale sqrt(t) or attention magnitude (default: %.1f)",
    ...
    add_opt(common_arg({ "-gan", "--grp-attn-n" }, "N",
        string_format("group-attention factor (default: %d)", params.grp_attn_n),
                ...
                .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_PASSKEY }));
    add_opt(common_arg({ "-gaw", "--grp-attn-w" }, "N",
        string_format("group-attention width (default: %d)", params.grp_attn_w),
                ...
                .set_examples({ LLAMA_EXAMPLE_MAIN }));
    add_opt(common_arg({ "-nkvo", "--no-kv-offload" }, "disable KV offload", [](common_params & params) {
            params.no_kv_offload = true;
            }).set_env("LLAMA_ARG_NO_KV_OFFLOAD"));
    add_opt(common_arg({ "-nr", "--no-repack" }, "disable weight repacking", [](common_params & params) {
    ...
    add_opt(common_arg({ "-cb", "--cont-batching" },
                       string_format("enable continuous batching (a.k.a dynamic batching) (default: %s)",
                                     params.cont_batching ? "enabled" : "disabled"),
                       [](common_params & params) { params.cont_batching = true; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_CONT_BATCHING"));
    add_opt(common_arg({ "-nocb", "--no-cont-batching" }, "disable continuous batching",
                       [](common_params & params) { params.cont_batching = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_NO_CONT_BATCHING"));
    add_opt(common_arg({ "--mmproj" }, "FILE",
        "path to a multimodal projector file. see tools/mtmd/README.md\n"
        "note: if -hf is used, this argument can be omitted",
                       [](common_params & params, const std::string & value) { params.mmproj.path = value; })
                .set_examples(mmproj_examples)
                .set_env("LLAMA_ARG_MMPROJ"));
    add_opt(common_arg({ "--mmproj-url" }, "URL", "URL to a multimodal projector file. see tools/mtmd/README.md",
                       [](common_params & params, const std::string & value) { params.mmproj.url = value; })
                .set_examples(mmproj_examples)
                .set_env("LLAMA_ARG_MMPROJ_URL"));
    add_opt(common_arg({ "--no-mmproj" }, "explicitly disable multimodal projector, useful when using -hf",
                       [](common_params & params) { params.no_mmproj = true; })
                .set_examples(mmproj_examples)
                .set_env("LLAMA_ARG_NO_MMPROJ"));
    add_opt(common_arg({ "--no-mmproj-offload" }, "do not offload multimodal projector to GPU",
                       [](common_params & params) { params.mmproj_use_gpu = false; })
                .set_examples(mmproj_examples)
                .set_env("LLAMA_ARG_NO_MMPROJ_OFFLOAD"));
    add_opt(
        common_arg(
        {"--image", "--audio"}, "FILE",
        "path to an image or audio file. use with multimodal models, can be repeated if you have multiple files\n",
            [](common_params & params, const std::string & value) { params.image.emplace_back(value); })
    ...
    if (llama_supports_rpc()) {
        add_opt(common_arg({ "--rpc" }, "SERVERS", "comma separated list of RPC servers",
            [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "--no-mmap" },
        "do not memory-map model (slower load but may reduce pageouts if not using mlock)",
                       [](common_params & params) { params.use_mmap = false; })
    ...
    add_opt(common_arg({ "--override-tensor", "-ot" }, "<tensor name pattern>=<buffer type>,...",
        "override tensor buffer type", [](common_params & params, const std::string & value) {
            parse_tensor_buffer_overrides(value, params.tensor_buft_overrides);
                       }));
    add_opt(common_arg({ "--override-tensor-draft", "-otd" }, "<tensor name pattern>=<buffer type>,...",
    ...
    add_opt(
        common_arg({ "--cpu-moe", "-cmoe" }, "keep all Mixture of Experts (MoE) weights in the CPU",
                   [](common_params & params) { params.tensor_buft_overrides.push_back(llm_ffn_exps_cpu_override()); })
            .set_env("LLAMA_ARG_CPU_MOE"));
    add_opt(common_arg({ "--n-cpu-moe", "-ncmoe" }, "N",
    ...
    add_opt(common_arg({ "--cpu-moe-draft", "-cmoed" },
    ...
    add_opt(common_arg({ "--n-cpu-moe-draft", "-ncmoed" }, "N",
    ...
    add_opt(common_arg({ "-ngl", "--gpu-layers", "--n-gpu-layers" }, "N",
    ...
    add_opt(common_arg({ "-ts", "--tensor-split" }, "N0,N1,N2,...",
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (split_arg.size() >= llama_max_devices()) {
                throw std::invalid_argument(
                                   string_format("got %d input configs, but system only has %d devices",
                                                 (int) split_arg.size(), (int) llama_max_devices()));
            }
    ...
    add_opt(common_arg({ "--no-op-offload" },
                       string_format("disable offloading host tensor operations to device (default: %s)",
                                     params.no_op_offload ? "true" : "false"),
                       [](common_params & params) { params.no_op_offload = true; }));
    add_opt(
        common_arg(
    ...
    add_opt(
        common_arg({ "-hfd", "-hfrd", "--hf-repo-draft" }, "<user>/<model>[:quant]",
        "Same as --hf-repo, but for the draft model (default: unused)",
                   [](common_params & params, const std::string & value) { params.speculative.model.hf_repo = value; })
    ...
    add_opt(common_arg({ "-hft", "--hf-token" }, "TOKEN",
        "Hugging Face access token (default: value from HF_TOKEN environment variable)",
                ...
                .set_env("HF_TOKEN"));
    add_opt(common_arg({ "--context-file" }, "FNAME", "file to load context from (repeat to specify multiple files)",
        [](common_params & params, const std::string & value) {
            std::ifstream file(value, std::ios::binary);
            if (!file) {
                               throw std::runtime_error(
    ...
    add_opt(common_arg({ "--no-webui" },
        string_format("Disable the Web UI (default: %s)", params.webui ? "enabled" : "disabled"),
                       [](common_params & params) { params.webui = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_NO_WEBUI"));
    add_opt(
        common_arg(
    ...
    add_opt(common_arg({ "--reranking", "--rerank" },
    ...
    add_opt(common_arg({ "--api-key" }, "KEY", "API key to use for authentication (default: none)",
                       [](common_params & params, const std::string & value) { params.api_keys.push_back(value); })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(common_arg({ "--cache-reuse" }, "N",
                       string_format("min chunk size to attempt reusing from the cache via KV shifting (default: %d)\n"
    ...
    add_opt(common_arg({ "--slots" },
                       string_format("enable slots monitoring endpoint (default: %s)",
                                     params.endpoint_slots ? "enabled" : "disabled"),
                       [](common_params & params) { params.endpoint_slots = true; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_ENDPOINT_SLOTS"));
    add_opt(common_arg({ "--no-slots" }, "disables slots monitoring endpoint",
                       [](common_params & params) { params.endpoint_slots = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_NO_ENDPOINT_SLOTS"));
    ...
    add_opt(common_arg({ "--no-models-autoload" }, "disables automatic loading of models (default: enabled)",
                       [](common_params & params) { params.models_autoload = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_NO_MODELS_AUTOLOAD"));
    add_opt(common_arg(
        {"--jinja"},
        string_format("use jinja template for chat (default: %s)\n", params.use_jinja ? "enabled" : "disabled"),
                [](common_params & params) { params.use_jinja = true; })
                .set_examples({ LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_MTMD })
                .set_env("LLAMA_ARG_JINJA"));
    add_opt(common_arg({ "--no-jinja" },
                       string_format("disable jinja template for chat (default: %s)\n",
                                     params.use_jinja ? "enabled" : "disabled"),
                       [](common_params & params) { params.use_jinja = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_MTMD })
                .set_env("LLAMA_ARG_NO_JINJA"));
    ...
    add_opt(
        common_arg({ "--chat-template" }, "JINJA_TEMPLATE",
            ...
            .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_MTMD })
    ...
    add_opt(common_arg({ "--no-prefill-assistant" },
                       ...
                       [](common_params & params) { params.prefill_assistant = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_NO_PREFILL_ASSISTANT"));
    ...
    add_opt(common_arg({ "--poll-batch-draft" }, "<0|1>",
        "Use polling to wait for draft model work (default: --poll-draft)",
                ...
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE }));
    add_opt(common_arg({ "--draft-max", "--draft", "--draft-n" }, "N",
                       string_format("number of tokens to draft for speculative decoding (default: %d)",
    ...
    add_opt(common_arg({ "--draft-min", "--draft-n-min" }, "N",
    ...
    add_opt(common_arg({ "--draft-p-split" }, "P",
                       string_format("speculative decoding split probability (default: %.1f)",
    ...
    add_opt(common_arg(
        {"--draft-p-min"}, "P",
                string_format("minimum speculative decoding probability (greedy) (default: %.1f)",
    ...
    add_opt(common_arg({ "-md", "--model-draft" }, "FNAME", "draft model for speculative decoding (default: unused)",
                       [](common_params & params, const std::string & value) { params.speculative.model.path = value; })
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER })
