...
static struct llama_model * llama_model_load_from_file_impl(const std::string &        path_model,
                                                            std::vector<std::string> & splits,
                                                            struct llama_model_params  params) {
    ggml_time_init();

    // Log model path and parameters
    LLAMA_LOG_INFO("%s: loading model from '%s'\n", __func__, path_model.c_str());

    // Log all splits if any
    if (!splits.empty()) {
        LLAMA_LOG_INFO("%s: loading %zu split(s):\n", __func__, splits.size());
        for (size_t i = 0; i < splits.size(); ++i) {
            LLAMA_LOG_INFO("%s:   split %zu: '%s'\n", __func__, i, splits[i].c_str());
        }
    }

    // Log all model parameters
    LLAMA_LOG_INFO("%s: n_gpu_layers = %d\n", __func__, params.n_gpu_layers);
    LLAMA_LOG_INFO("%s: split_mode = %d\n", __func__, params.split_mode);
    LLAMA_LOG_INFO("%s: main_gpu = %d\n", __func__, params.main_gpu);
    LLAMA_LOG_INFO("%s: tensor_split provided = %s\n", __func__, params.tensor_split ? "true" : "false");
    LLAMA_LOG_INFO("%s: progress_callback provided = %s\n", __func__, params.progress_callback ? "true" : "false");
    LLAMA_LOG_INFO("%s: vocab_only = %s\n", __func__, params.vocab_only ? "true" : "false");
    LLAMA_LOG_INFO("%s: use_mmap = %s\n", __func__, params.use_mmap ? "true" : "false");
    LLAMA_LOG_INFO("%s: use_mlock = %s\n", __func__, params.use_mlock ? "true" : "false");
    LLAMA_LOG_INFO("%s: check_tensors = %s\n", __func__, params.check_tensors ? "true" : "false");

    // Log tensor_split details if provided
    if (params.tensor_split) {
        std::string split_info = "";
        for (int i = 0; i < llama_max_devices(); i++) {
            if (params.tensor_split[i] > 0.0f) {
                split_info += std::to_string(params.tensor_split[i]);
                if (i < llama_max_devices() - 1) {
                    split_info += ", ";
                }
            } else {
                break;
        ...
        LLAMA_LOG_INFO("%s: tensor_split values = [%s]\n", __func__, split_info.c_str());
    }
    ...
    if (params.devices) {
        LLAMA_LOG_INFO("%s: using explicitly provided devices\n", __func__);
        for (ggml_backend_dev_t * dev = params.devices; *dev; ++dev) {
            model->devices.push_back(*dev);
        }
    } else {
        LLAMA_LOG_INFO("%s: using auto-detected devices\n", __func__);
        ...
        for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
            ...
            switch (ggml_backend_dev_type(dev)) {
                ...
                case GGML_BACKEND_DEVICE_TYPE_ACCEL:
                    ...
                    LLAMA_LOG_INFO("%s: skipping CPU/ACCEL backend device %zu: %s\n", __func__, i,
                ...
                case GGML_BACKEND_DEVICE_TYPE_GPU:
                    ...
                    if (ggml_backend_reg_name(reg) == std::string("RPC")) {
                        LLAMA_LOG_INFO("%s: found RPC backend device %zu: %s\n", __func__, i,
                                       ggml_backend_dev_name(dev));
                        rpc_servers.push_back(dev);
                    } else {
                        LLAMA_LOG_INFO("%s: found GPU backend device %zu: %s\n", __func__, i,
                                       ggml_backend_dev_name(dev));
                        model->devices.push_back(dev);
                    }
                    break;
        ...
        if (!rpc_servers.empty()) {
            LLAMA_LOG_INFO("%s: prioritizing %zu RPC server(s)\n", __func__, rpc_servers.size());
            model->devices.insert(model->devices.begin(), rpc_servers.begin(), rpc_servers.end());
        }
    }

    LLAMA_LOG_INFO("%s: total devices: %zu\n", __func__, model->devices.size());
    ...
    if (params.split_mode == LLAMA_SPLIT_MODE_NONE) {
        ...
        LLAMA_LOG_INFO("%s: using single GPU mode with device %d\n", __func__, params.main_gpu);
    }

    for (size_t i = 0; i < model->devices.size(); i++) {
        auto * dev = model->devices[i];
        size_t free, total;  // NOLINT
        ggml_backend_dev_memory(dev, &free, &total);
        LLAMA_LOG_INFO("%s: device %zu: %s (%s) - %zu MiB free of %zu MiB total\n", __func__, i,
                       ggml_backend_dev_name(dev), ggml_backend_dev_description(dev), free / 1024 / 1024,
                       total / 1024 / 1024);
    }
...
struct llama_context * llama_init_from_model(
    ...
    const uint32_t n_ctx_per_seq = cparams.n_ctx / cparams.n_seq_max;
      ...
      // Log all cparams values
    LLAMA_LOG_INFO("%s: n_seq_max       = %u\n", __func__, cparams.n_seq_max);
    LLAMA_LOG_INFO("%s: n_ctx           = %u\n", __func__, cparams.n_ctx);
    LLAMA_LOG_INFO("%s: n_ctx_per_seq   = %u\n", __func__, n_ctx_per_seq);
    LLAMA_LOG_INFO("%s: n_batch         = %u\n", __func__, cparams.n_batch);
    LLAMA_LOG_INFO("%s: n_ubatch        = %u\n", __func__, cparams.n_ubatch);
    LLAMA_LOG_INFO("%s: n_threads       = %d\n", __func__, cparams.n_threads);
    LLAMA_LOG_INFO("%s: n_threads_batch = %d\n", __func__, cparams.n_threads_batch);
    LLAMA_LOG_INFO("%s: rope_freq_base  = %.1f\n", __func__, cparams.rope_freq_base);
    LLAMA_LOG_INFO("%s: rope_freq_scale = %g\n", __func__, cparams.rope_freq_scale);
    LLAMA_LOG_INFO("%s: n_ctx_orig_yarn = %u\n", __func__, cparams.n_ctx_orig_yarn);
    LLAMA_LOG_INFO("%s: yarn_ext_factor = %g\n", __func__, cparams.yarn_ext_factor);
    LLAMA_LOG_INFO("%s: yarn_attn_factor= %g\n", __func__, cparams.yarn_attn_factor);
    LLAMA_LOG_INFO("%s: yarn_beta_fast  = %g\n", __func__, cparams.yarn_beta_fast);
    LLAMA_LOG_INFO("%s: yarn_beta_slow  = %g\n", __func__, cparams.yarn_beta_slow);
    LLAMA_LOG_INFO("%s: defrag_thold    = %g\n", __func__, cparams.defrag_thold);
    LLAMA_LOG_INFO("%s: embeddings      = %d\n", __func__, cparams.embeddings);
    LLAMA_LOG_INFO("%s: causal_attn     = %d\n", __func__, cparams.causal_attn);
    LLAMA_LOG_INFO("%s: offload_kqv     = %d\n", __func__, cparams.offload_kqv);
    LLAMA_LOG_INFO("%s: flash_attn      = %d\n", __func__, cparams.flash_attn);
    LLAMA_LOG_INFO("%s: no_perf         = %d\n", __func__, cparams.no_perf);
    LLAMA_LOG_INFO("%s: pooling_type    = %d\n", __func__, cparams.pooling_type);

    // Log additional params values that might not be in cparams
    LLAMA_LOG_INFO("%s: rope_scaling_type = %d\n", __func__, params.rope_scaling_type);
    LLAMA_LOG_INFO("%s: attention_type    = %d\n", __func__, params.attention_type);
    LLAMA_LOG_INFO("%s: type_k            = %d\n", __func__, params.type_k);
    LLAMA_LOG_INFO("%s: type_v            = %d\n", __func__, params.type_v);
    LLAMA_LOG_INFO("%s: logits_all        = %d\n", __func__, params.logits_all);
    LLAMA_LOG_INFO("%s: abort_callback    = %p\n", __func__, (void *) params.abort_callback);
    LLAMA_LOG_INFO("%s: abort_callback_data = %p\n", __func__, params.abort_callback_data);
