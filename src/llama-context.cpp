...
// Instead of returning the whole cache, return just the cells pointer and size
bool llama_get_kv_cells(struct llama_context * ctx, uint32_t* size_out, struct llama_kv_cell** cells_out) {
    if (!ctx || !size_out || !cells_out) {
        return false;
    }
    
    auto cache = ctx->kv_self.get();
    if (!cache) {
        return false;
    }
    
    *size_out = cache->size;
    *cells_out = cache->cells.data();
    return true;
}
